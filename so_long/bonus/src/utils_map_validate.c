...
/*   Updated: 2025/09/14 20:58:37 by gbercaco         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	flood_fill(char **map, int y, int x, t_game *game)
{
	if (y < 0 || y >= game->map->height || x < 0 || x >= game->map->width)
		return ;
	if (map[y][x] == '1' || map[y][x] == 'F' || map[y][x] == 'O'
		|| map[y][x] == '5' || map[y][x] == 'U')
		return ;
	map[y][x] = 'U';
...
void	draw_moves(t_game *game)
{
	...
	if (game->images.moves_img)
		mlx_delete_image(game->mlx, game->images.moves_img);
	moves_str = ft_itoa(game->moves);
	if (!moves_str)
		return ;
	game->images.moves_img = mlx_put_string(game->mlx, moves_str, 70, 10);
	free(moves_str);
}
